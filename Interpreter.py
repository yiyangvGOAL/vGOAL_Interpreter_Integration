...
# --- Performance infrastructure (behaviour-preserving; mirrors the test version) ---
# Fast deep-copy for plain dict/list/tuple with immutable leaves; identical in
# value to copy.deepcopy but without its memo bookkeeping. Unrecognised types
# fall back to the stdlib, so it is always correct.
...
# Memoize state-independent rule instantiation (pure; returns an independent
# deep copy each call so callers see identical behaviour, only far fewer times).
_MISS = object()
_MEMO_REGISTRY = []
...
def transform_vGOAL_to_CNF(rules, atoms, generated_atoms, keywords):
    ...
    for key in minimal_Generation_dict:
        for r in minimal_Generation_dict[key]:
            # Skip an empty clause (arises from an empty-body rule "implies h",
            # i.e. an unconditional fact): [] is always UNSAT and h needs no
            # minimality constraint. Without this, empty-body rules (e.g. an
            # enableness rule after its action-constraint predicate is stripped)
            # make the whole CNF spuriously UNSAT.
            if r != [] and r not in CNF_Clauses:
...
# SAT-based cross-check of the least-fixpoint derivation (mirrors the test version).
_SAT_FAIL_COUNT = 0
...
# State-independent (verified byte-identical to the test version), so memoized.
@_memoize_pure
...
def predicate_existential_variables_instantiation(atoms, predicate, constants):
    ...
    if not predicate["list_contain"]:
        predicate_copy = _fast_deepcopy(predicate)
        for atom in atoms:
            # Arity guard: only unify with atoms of the same argument count (a
            # same-name atom of different arity is a different predicate; without
            # this the index accesses below raise IndexError on the shorter list).
        ...
        for constraint_copy in instantiated_constraints:
            ...
            for cons in Cons:
                ...
                if generated_info != []:
                    ...
                    # (communication messages are not propositional atoms, so no SAT
                    # cross-check here — matches the test version.)
...
def state_transformer(enable_actions, current_state, knowledge_base, action_specification, domain, constants):
    ...
    for rule in instantiated_constraints:
        ...
        # so skip it (guards against a NameError on current_enabled_action).
...
def interpreter(agents, knowledge_base, constraints_of_action_generation,
                 ...
                 prior_beliefs):
    _clear_memoization_caches()  # fresh caches per run
